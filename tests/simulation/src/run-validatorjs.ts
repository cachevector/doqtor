/**
 * End-to-end simulation: "validatorjs" scenario.
 *
 * A schema validation library where the maintainer merged a PR that:
 ...
 *   - Changed `formatErrors` signature from (errors, separator) to (errors, options?)
 *   - Expanded `isEmpty` to handle empty objects
 *
...
async function main() {
  console.log(`${BOLD}Doqtor End-to-End Simulation: validatorjs${RESET}`);
  console.log(`${DIM}Simulating a breaking change in "validatorjs" schema validation library${RESET}`);
  ...
  const files: Record<string, Record<string, string>> = {
    old: {
      "src/validator.ts": BEFORE_VALIDATOR,
      "src/utils.ts": BEFORE_UTILS,
    },
    new: {
      "src/validator.ts": AFTER_VALIDATOR,
      "src/utils.ts": AFTER_UTILS,
    },
  };

  const changeSets = await analyzeDiff({
    diff: VALIDATORJS_DIFF,
  ...
  const docFiles = [
    { path: "README.md", content: VALIDATORJS_README },
    { path: "docs/guide.md", content: VALIDATORJS_GUIDE },
  ];
  ...
  console.log(`${BOLD}Branch:${RESET} doqtor/docs-update-pr-99`);
  console.log(`${BOLD}Title:${RESET}  docs: update after #99`);
  ...
  console.log();
  console.log(`This PR updates documentation to reflect code changes from #99.`);
  console.log();
  console.log(`### Changes`);
  console.log();
