...
describe("parser edge cases", () => {
  ...
  it("handles async functions", () => {
    ...
    const symbols = parseSource("async.ts", code);
  ...
  it("handles generic functions", () => {
    ...
    const symbols = parseSource("generic.ts", code);
...
export function process(x: string | number): string | number {
  return x;
    ...
    const symbols = parseSource("overload.ts", code);
  ...
  constructor(private name: string) {}
    ...
    const symbols = parseSource("ctor.ts", code);
  ...
  it("handles enum (not extracted — expected behavior)", () => {
    ...
    const symbols = parseSource("enum.ts", code);
  ...
  nested: {
    ...
    const symbols = parseSource("nested.ts", code);
